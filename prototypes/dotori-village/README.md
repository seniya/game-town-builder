...
주민 30명이 과장된 성격대로 알아서 사는 작은 마을이다. 본 게임(`src/`)과 떨어진, 버려도 되는 실험이다.
...
style.css       공용 스타일
assets/         3D 모델. 데이터를 안에 품은 glTF JSON (.json). 아티팩트가 .glb 를 서빙하지 않기 때문이다
tools/pack_assets.py    내려받은 CC0 팩에서 쓸 모델만 골라 assets/ 를 만든다
```
...
ES 모듈이라 `file://` 로는 열리지 않는다. 이 폴더에서 정적 서버를 띄운다.

```text
python3 -m http.server 8765    → http://localhost:8765/index.html , /3d.html
...
라이선스 원문은 `assets/LICENSE-*.txt` 에 있다. 조사 기록: [docs/research/2026-09-26-cc0-3d-asset-packs.md](../../docs/research/2026-09-26-cc0-3d-asset-packs.md)
