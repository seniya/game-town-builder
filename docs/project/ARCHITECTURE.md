...
구현(TASK-047): NPCDecisionDeps.threat 가 위협(반경 12, 도피 중에는 15)과 도피 칸(GameWorld 가 정하고 위협이 풀릴 때까지 유지)을 준다.
flee 계획은 도피 칸으로 MoveAction(kind 'flee', 속도 5.0), 도착하면 '숨는 중' 대기다. 기절은 '기절' 대기(주저앉기)다(TASK-046).
...
구현(PERF-002, ADR 037): FarmSystem / RepairSystem 은 판(변경 revision)과 5 게임분 시각 칸이 같으면 주민별 지난 후보를 쓰고,
이미 칸을 잡고 일하러 가는 농부에게는 그 칸을 그대로 준다. NPCSystem 은 같은 프레임에 다른 주민이 먼저 잡은 밭 칸·시설을 목표로 하는
계획을 시작하지 않고 실패 간격(0.5 초) 뒤 다시 판단한다. 후보 선정은 여전히 후보 수에 비례하므로 장기 대형 월드에서는 공간 인덱스가 필요하다.
...
`types` 에 둔다. markDialogueAvailable 의 npcId 는 화자 키(ADR 035)다. playCutscene 은 CUTSCENE_REQUESTED 로 알린다. 엔딩(slice_end)은 EndingSystem 이 pending 으로 받고, 화면의 `ending` 모달이 연출한다(TASK-052, ADR 039).
...
몬스터의 쿨다운·파괴 진행도는 로드 뒤 처음부터 다시 판단한다. dialogue 는 { completed, available } 로 묶었다. ending 은 `systems/EndingSystem.ts` 의 { pending, played } 다(TASK-052). 엔딩을 본 직후에도 저장을 요청한다.
...
043  오너 확인 반영: 팔 방향 기준·사거리 8·잎은 천장 아님·BGM 끔 Accepted
044  미술 방향: 블록 세계 위 둥글고 귀여운 주민·소품(방향)  Accepted
045  시안 모형: 코드 생성 둥근 지오메트리·관절별 병합·툰 명암·외곽선  Accepted
046  X0 진행: V1 오너 위임, 그림자·톤 매핑·빛 번짐·하늘 렌더 파이프라인  Accepted
047  가구·소품을 인스턴싱 둥근 모형으로(모양 표에서)            Accepted
048  오너 확인 2 차: 건너뛴 밤의 습격은 다음 21:00·나침반 HUD    Accepted
```
