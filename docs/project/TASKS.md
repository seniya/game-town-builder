...
10. STYLE-001 → (게이트 V1: 오너 위임, 에이전트 선택) → STYLE-005 → STYLE-004 → STYLE-002 → STYLE-003 → STYLE-007 → STYLE-006 (게이트 V2: 2026-09-25 통과) → STYLE-008 (다듬기, 계속) (2026-09-25 사용자 결정 D1: [확장 로드맵](EXPANSION_ROADMAP.md) X0 부터. 명세 MVP_SPEC 45. 순서와 V1 위임은 ADR 046)
...
결과: **완료** (2026-09-25 오너 통과 판정). [TASK-053 기록](../state/2026-09-25_0600-task-053-balance-perf.md), [통과 기록](../state/2026-09-25_1930-task-053-owner-pass.md). 자동 검산·측정 AC 는 에이전트가 통과시켰고,
사람 플레이 AC 는 오너가 HR-029 를 통과로 판정해 닫았다. 첫 방·첫 취침·전체 실제 소요 시간과 실제 창 FPS 수치는 기록되지 않았다.
플레이 중 의견(BGM 끔, 가구·아이템 식별성)은 HR-029·HR-014 와 ADR 043 에 남겼다.
...
- [x] 첫 방 10~20분·첫 취침 20~30분·전체 90~120분 가설과 실제 소요를 비교한다. 차이와 대기·채집·진단의 원인을 기록하고 분량만 맞추기 위한 지연을 추가하지 않는다 — 2026-09-25 오너 통과 판정(HR-029)으로 닫음. 소요 시간 수치는 기록되지 않았다
- [x] 분량 가설이 빗나가면 재미·진행 결과에 근거해 명세 유지/개정 판단을 기록한다. 측정 전 목표 분량 달성을 선언하지 않는다 — 2026-09-25 오너 통과 판정(HR-029)으로 닫음. 소요 시간 수치는 기록되지 않았다
...
- [x] MVP_SPEC 39 장의 Acceptance Test 10 개가 전부 통과한다 — 자동 단계는 시험으로, 시각·청각 단계는 2026-09-25 오너 통과 판정(HR-029)으로
...
- [x] 방을 더 꾸미고 싶은지, 주민에 정감이 가는지, 밤 귀가를 다시 보고 싶은지 기록한다. 큰 마을을 한동안 바라보고 싶다는 기대는 장기 규모에서 재검증할 항목으로 남긴다 — 2026-09-25 오너 통과 판정. 세 질문에 대한 개별 답은 기록되지 않았다
...
상태: 완료 → HR-030 `고칠 곳 있음`(2026-09-25). 이어서 TASK-ANIM-002.
...
2026-09-25 오너가 V1 의 선택을 맡겼다("X0 을 최신 그래픽 느낌에 맞게 알아서 진행해보자"). 에이전트가 고른 값은
MVP_SPEC 45.2 에 있고, 오너의 확인은 V2 에서 받는다(ADR 046).

---

## TASK-STYLE-005 렌더 품질: 그림자·톤 매핑·빛 번짐·하늘·물·재질 규약

의존: STYLE-001, ADR 046. 명세 MVP_SPEC 45.3. X0 의 렌더 바탕이라 먼저 한다. **게임 상태·저장을 바꾸지 않는다.**

작업:

```text
render/Renderer.ts: 그림자 지도(카메라 주변 추적·텍셀 맞춤), EffectComposer(장면 → 빛 번짐 → 출력 → 색 보정), ?quality
render/materials.ts: 복셀 셰이더에 그림자 받기·물 셰이딩·창문 불빛, 걷어 내기 깊이 재질, 하늘 돔 재질, 색 보정 셰이더,
                     사람·가구 재질의 밝기 규약(π) 공통화
render/SkyDome.ts: 하늘 돔(그라데이션·해 광륜·구름·별·달)
render/DayNightVisual.ts: 천정색·지평선색·그림자 세기 키, 하늘 돔과 안개 갱신
```

Acceptance Criteria:

- [x] 블록·사람·가구·작물이 해 그림자를 던지고 받는다. 물·창문·오버레이는 던지지 않는다(headless 관찰)
- [x] 걷힌 천장은 그림자를 던지지 않는다(집 안 headless 관찰)
- [x] 톤 매핑·빛 번짐·색 보정이 적용되고 `?quality=low` 는 그림자·빛 번짐을 끈다(시험·관찰)
- [x] 하늘 돔이 시각별로 천정·지평선 색을 바꾸고 밤에 별이 보인다. 안개색이 지평선색과 같다(시험·관찰)
- [x] 캐릭터·가구 Lambert 재질이 블록과 같은 밝기 규약을 쓴다(시험)
- [x] 초기 섬·집·주방 장면 드로우콜이 600 미만이고 headless 60 FPS 를 유지한다(계측)

결과: [STYLE-005 기록](../state/2026-09-25_2230-style-005-render-quality.md). 시험 `tests/renderQuality.test.ts`.

## TASK-STYLE-004 블록 텍스처·팔레트

의존: STYLE-005. 명세 MVP_SPEC 45.5.

Acceptance Criteria:

- [x] 기존 블록 22 종 전부가 64 px 손그림풍 타일을 갖는다. 타일은 결정적이다(시험)
- [x] 텍스처 배열(층 = 타일)·선형 필터·밉맵으로 그리고, 병합 쿼드의 반복 경계에 줄이 생기지 않는다(관찰)
- [x] 인벤토리 아이콘이 새 타일을 쓴다
- [x] 방 진단 오버레이·조준 테두리·균열이 새 텍스처 위에서 읽힌다(관찰)

결과: [STYLE-004 기록](../state/2026-09-25_2240-style-004-block-tiles.md). 시험 `tests/blockTiles.test.ts`.

## TASK-STYLE-002 주민·플레이어 모형 전환

의존: STYLE-005, 45.2 선택. 명세 MVP_SPEC 45.4.

Acceptance Criteria:

- [x] 역할 넷(농부·요리사·목수·주민)과 플레이어가 둥근 SD 모형(2.5 등신·큰 눈)으로 그려지고 역할 소품으로 구별된다
- [x] 한 명이 강체 스키닝 한 메시 + 외곽선 한 메시다(그림자 포함 드로우콜 3 이하, 시험)
- [x] 자세 표(ADR 041) 전부를 입고, 의자 앉기·침대 눕기·밭일에서 몸이 가구·땅에 맞는다(관찰)
- [x] 눈 깜빡임·잠든 눈·누울 때 모자 벗기·이불·z·김·대화 표시가 그대로 동작한다
- [x] 몸체 AABB·문·침대 크기와 게임 상태는 그대로다

결과: [STYLE-002 기록](../state/2026-09-25_2250-style-002-characters.md). 시험 `tests/cuteCharacter.test.ts`.

## TASK-STYLE-003 가구·소품 모형과 아이콘

의존: STYLE-002. 명세 MVP_SPEC 45.6.

Acceptance Criteria:

- [x] 침대·문·횃불·식탁·의자·화덕·물 항아리·상자·종·작물이 기능이 읽히는 둥근 모형으로 그려진다(창문은 벽 블록으로 남는다)
- [x] 모형은 종류별 InstancedMesh(+ 외곽선)라 가구 수와 드로우콜이 무관하다(시험)
- [x] 의자 등받이·침대·문 방향 규칙이 지금과 같다(시험)
- [x] 충돌·통행·방 판정·조준·설치는 바뀌지 않는다(기존 시험 통과)
- [x] 가구 인벤토리 아이콘을 같은 모형에서 그린다

결과: [STYLE-003 기록](../state/2026-09-25_2310-style-003-furniture.md), [ADR 047](../adr/047-furniture-instanced-models.md). 시험 `tests/furnitureView.test.ts`.

## TASK-STYLE-007 UI 톤·몬스터 모형

의존: STYLE-002. 명세 MVP_SPEC 45.7.
...
## TASK-STYLE-008 다듬기 1 차: 풀잎 장식·풀 무늬·눕기·불꽃 번짐

의존: STYLE-006(V2 통과). 명세 MVP_SPEC 45.8. 오너 지시 "지속적으로 업그레이드 하자". **게임 상태·저장을 바꾸지 않는다.**

Acceptance Criteria:

- [x] 풀 블록 윗면(위가 air)에만 풀잎 포기가 결정적으로 놓이고, 카메라 반경 30 칸 밖은 그리지 않는다(시험)
- [x] 풀잎은 InstancedMesh 하나이고 블록이 바뀐 청크만 다시 모은다(시험)
- [x] 풀 윗면 타일의 포기 무늬가 줄었다(타일 시트 관찰)
- [x] 누운 주민의 머리가 베개 위에 얹힌다(headless 관찰)
- [x] 불꽃 밝기 배율이 낮보다 밤에 크다(시험)
- [x] 36 장 예산 안이다(계측)

결과: [STYLE-008 기록](../state/2026-09-25_2400-style-008-polish-1.md). 시험 `tests/grassView.test.ts`.

## TASK-REVIEW-002 오너 확인 2 차 반영: 건너뛴 밤의 습격·나침반·관찰 장면 절차

의존: 확인판 기록(2026-09-25 23:16~23:31). 결정 [ADR 048](../adr/048-owner-review-2-raid-skip-compass.md). 명세 MVP_SPEC 20.3·29.

Acceptance Criteria:

- [x] 시각 강제 설정이 21:00~05:00 을 통째로 건너뛰면 습격이 낮에 열리지 않고 다음 21:00 로 미뤄진다. 밤 안에서 멈추면 그 자리에서 열린다(시험 `tests/raid.test.ts`)
- [x] 화면 위 가운데 나침반이 카메라 방위의 동서남북을 보여 준다(북 = −z, 동 = +x. 시험 `tests/compass.test.ts`, headless 관찰)
- [x] `?scene=kitchen-lab` 의 침실 침대가 둘이라 "감사 +20" 네 번만으로 레벨 2 종을 칠 수 있다(headless 계측: housingLevel 67)
- [x] 밭일 굽힘이 오르내려 정면에서도 읽힌다(headless 관찰, 사람 판단은 HR-013)
- [x] HR-019·021 의 장면을 번호 붙인 단계로 다시 쓴다

## TASK-ANIM-002 행동별 동작 다듬기 (HR-030)

의존: TASK-ANIM-001(HR-030 `고칠 곳 있음`), STYLE-002(둥근 SD 모형). 상태: **미착수**.
오너 판단(2026-09-25): "각각의 행동의 애니메이션이 더 필요하다. 정교함이 필요하다." **렌더 표현만 바꾼다**(ANIM-001 과 같은 원칙).
새 Action·자세 종류를 게임 상태에 더하지 않는다.

Acceptance Criteria:

- [ ] 행동마다(걷기·달리기·밭일·조리·식사·취침·수리·기절·대화·대기) 준비 → 반복 → 마무리의 결이 있다: 한 자세의 고정 반복이 아니라 몸 전체(머리·몸통·팔·다리)가 함께 움직인다
- [ ] 걷기·달리기에 몸 기울기·머리 흔들림·발 디딤의 튐이 있다(STYLE-008 다음 할 일)
- [ ] 큰 머리 SD 모형에서 정면·옆 어느 쪽에서 봐도 행동이 구별된다(headless 네 방향 관찰)
- [ ] 자세 표·보간 시험이 계속 통과하고 드로우콜·프레임이 늘지 않는다
- [ ] 사람 판단은 HR-030 재확인으로 받는다
