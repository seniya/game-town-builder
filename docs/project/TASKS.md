...
구현 전: ADR 028 에 방식(메셔 모양 표 / 모형)·AO·면 컬링·성능 기준을 보완하고 아래 AC 를 구체화한다.
...
- [ ] 충돌·통행·방 판정·설치·드롭 테스트가 바뀌지 않고 모두 통과한다
- [ ] 모양 블록을 많이 놓아도 청크 드로우콜이 늘지 않는다(모양 표 방식이면 청크 메시 안에 포함)
- [ ] 브라우저에서 가구 모양·주민 사용 자세를 확인하고, 사람 확인은 HUMAN_REVIEW 에 올린다
