...
- [ ] 역할 넷(농부·요리사·목수·주민)과 플레이어가 둥근 SD 모형(2.5 등신·큰 눈)으로 그려지고 역할 소품으로 구별된다
- [ ] 한 명이 강체 스키닝 한 메시 + 외곽선 한 메시다(그림자 포함 드로우콜 3 이하, 시험)
- [ ] 자세 표(ADR 041) 전부를 입고, 의자 앉기·침대 눕기·밭일에서 몸이 가구·땅에 맞는다(관찰)
- [ ] 눈 깜빡임·잠든 눈·누울 때 모자 벗기·이불·z·김·대화 표시가 그대로 동작한다
- [ ] 몸체 AABB·문·침대 크기와 게임 상태는 그대로다
