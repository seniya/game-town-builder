...
- [ ] 블록·사람·가구·작물이 해 그림자를 던지고 받는다. 물·창문·오버레이는 던지지 않는다(headless 관찰)
- [ ] 걷힌 천장은 그림자를 던지지 않는다(집 안 headless 관찰)
- [ ] 톤 매핑·빛 번짐·색 보정이 적용되고 `?quality=low` 는 그림자·빛 번짐을 끈다(시험·관찰)
- [ ] 하늘 돔이 시각별로 천정·지평선 색을 바꾸고 밤에 별이 보인다. 안개색이 지평선색과 같다(시험·관찰)
- [ ] 캐릭터·가구 Lambert 재질이 블록과 같은 밝기 규약을 쓴다(시험)
- [ ] 초기 섬·집·주방 장면 드로우콜이 600 미만이고 headless 60 FPS 를 유지한다(계측)
