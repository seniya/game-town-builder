...
- [ ] 침대·문·횃불·식탁·의자·화덕·물 항아리·상자·종·작물이 기능이 읽히는 둥근 모형으로 그려진다(창문은 벽 블록으로 남는다)
- [ ] 모형은 종류별 InstancedMesh(+ 외곽선)라 가구 수와 드로우콜이 무관하다(시험)
- [ ] 의자 등받이·침대·문 방향 규칙이 지금과 같다(시험)
- [ ] 충돌·통행·방 판정·조준·설치는 바뀌지 않는다(기존 시험 통과)
- [ ] 가구 인벤토리 아이콘을 같은 모형에서 그린다
