...
- [ ] 전이가 갑자기 튀지 않고 보간된다
- [ ] 기존 torch를 놓은 작은 집에서 밤의 실내 불빛과 귀가하는 주민의 자세가 함께 읽힌다. 자동 점등 기능이나 광원 상한 확대 없이 확인한다
...
- [ ] 첫 종 대사가 레벨 3 게이트(인식된 방 4개, 침대, food 8)와 `chest`·`table`/`chair`로 창고·식당을 만들 수 있다는 것까지 안내한다 (MVP_SPEC 28 Phase 4)
