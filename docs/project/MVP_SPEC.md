...
모든 점유 칸에서 월드 범위·기존 객체·플레이어와 NPC AABB 충돌을 검사한다.
...
같은 대사에서 다음 종의 조건도 알려 준다. **인식된 방 4 개, 주민 수만큼의 침대,
food 8 이상** 이다. 새로 해금된 `chest` 로 창고를, 이미 있는 `table` / `chair` 로
...
저장은 프레임 끝의 커밋된 상태에서 수행한다. 미완료 Action은 자원 예약만 해제하고 재판단한다.
