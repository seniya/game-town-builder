...
> ```
>
> **게임 상태와 렌더를 분리하는 원칙은 유지한다.** 아래 예시의 `NPC.view`와
> MovementController가 View를 쓰는 방식은 현재 계약이 아니다.
> 현재 NPC는 View 참조를 소유하지 않고 render가 읽기 전용으로 동기화한다
> (ARCHITECTURE 12장, ADR 020). 엔진 변경은 설계 단계이며 실제 이전 비용은 미검증이다.
