...
관련 결정: ADR 011 / 014 / 019

> v1.3 문서 정합성 보정: 아래 외부 출처의 최신성은 이번에 다시 확인하지 않았다.
> 현재 선택은 ADR 019를 따른다. 드로우콜 수만으로 WebGPU로 전환하지 않으며,
> 기존 비교 글의 지원 범위·번들 크기·다운로드 수를 프로젝트 성능의 증거로 쓰지 않는다.
...
2026-09-22 보정: 다운로드 비교의 집계 범위와 번들 크기의 버전·압축·포함 기능은
재확인하지 않았다. 아래 비교는 자체 벤치마크나 동등 조건의 성능 보장이 아니다.

아래에는 외부 글의 주장, 단순 면 수 계산, 프로젝트의 선택이 섞여 있다.
외부 측정 조건은 재현하지 않았으며 우리 월드(128 × 64 × 128)의 실측은 없다.

TASK-005~008 / PERF-001·002 / 053의 실제 측정은 조건과 함께 별도로 기록한다.
조건이 다른 외부 수치를 우리 측정값으로 덮어쓰지 않고 비교 근거와 한계를 남긴다.
...
전환 비용을 낮추기 위해 재질 생성을 `src/render/materials.ts` 한 곳에 모으도록 설계했다. 구현은 아직 없다.
...
MVP는 청크 256개지만 실제 비용은 아직 측정하지 않았다.
ADR 019에 따라 드로우콜 증가는 근경·원경의 CPU/GPU 비용을 측정할 신호다.
배치·가시성·광원 예산을 먼저 검토하고 실제 GPU 병목이 확인된 경우에만 WebGPU를 검토한다.
...
| 사실 | 출처 | 우리 결정 |
|---|---|---|
| 다운로드 비교 수치 (집계 범위 미확인) | [c1] | 보조 정보이며 채택·성능의 단독 근거가 아님 — ADR 011 |
| 번들 비교 수치 (동등 빌드 조건 미확인) | [u1] | 보조 정보이며 자체 벤치마크가 아님 — ADR 011 |
| Babylon 은 Havok / 에디터 / 컴포넌트 모델을 준다 | [c1][u1] | 우리가 쓰지 않는 기능이라 판단 — ADR 011 |
| WebGPU 지원에 관한 비교 글의 주장 (대상 환경 재검증 필요) | [c1] | MVP는 WebGL2 유지 — ADR 011 / 019 |
...
| 청크가 많으면 드라이버 오버헤드 | [nm1] | CPU/GPU 비용 측정·배치 최적화 검토 — ADR 019 |
...
**네 번째 항목은 별도 측정이 필요하다.**

그리디 메싱의 병합 효과와 정점 AO 는 서로 충돌한다.
2.1의 6.25%는 내부 면 컬링 비율로 AO와 별개다.
AO는 그 다음 그리디 병합률을 낮출 수 있으며 두 감소량을 분리해 측정한다.
...
드로우콜이 2000 을 넘거나 GPU 프레임 타임이 예산의 절반을 넘는다
    →  CPU/GPU 비용을 분리 측정하고 ADR 019의 검토 조건을 적용한다

월드를 256 × 128 × 256 이상으로 키우려 한다
    →  2.2 / 2.4와 ARCHITECTURE 9.6의 스트리밍·배치·LOD 비용을 먼저 검토한다

TASK-053 에서 실제 수치를 측정한다
    →  외부 주장·단순 계산과 우리 측정을 구분해 조건·차이를 기록한다
```
